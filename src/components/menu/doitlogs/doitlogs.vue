...
<script>
  ...
  import detail from '@/components/menu/doitlogs/detail'
  import middle from '@/components/menu/doitlogs/middle'
  ...
  export default {
    ...
    created(){
      this.fields = config.table.doitlogs;
      this.loadData();
      // 根据当前界面宽高显示不同的字段
    },
    mounted(){
    ...
    methods:{
      loadData(){
        let url = tool.getURL(urls.doitlogs.list,this.search)
         axios.get(url,(res)=>{
            this.list = res.data;
      ...
      formatData(key,data){
        ...
        switch(key){
          case 'type':
            text = this.config.doitlogs[data[key]]
            break;
        }
      ...
      exportFile(){
        axios.download(urls.doitlogs.export,this.getName(),'xlsx')
      },
      getName(){
        let date = new Date();
        return  `用户列表`
      ...
      requestDel(id){
        let url = `${urls.doitlogs.delete}?id=${id}`;
