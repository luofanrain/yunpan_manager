...
<script>
  ...
  import detail from '@/components/menu/loginlogs/detail'
  import middle from '@/components/menu/loginlogs/middle'
  ...
  export default {
    ...
    created(){
      this.fields = config.table.loginlogs;
      this.loadData();
      // 根据当前界面宽高显示不同的字段
    },
    mounted(){
    ...
    methods:{
      loadData(){
        let url = tool.getURL(urls.loginlogs.list,this.search)
         axios.get(url,(res)=>{
            this.list = res.data;
      ...
      formatData(key,data){
        ...
        switch(key){
          case 'type':
            text = this.config.loginlogs[data[key]]
            break;
        }
      ...
      exportFile(){
        axios.download(urls.loginlogs.export,this.getName(),'xlsx')
      },
      getName(){
        let date = new Date();
        return  `用户列表`
      ...
      requestDel(id){
        let url = `${urls.loginlogs.delete}?id=${id}`;
